...
const _projectRoot = path.resolve(backendRoot, "..")
const backendRequire = createRequire(import.meta.url)
const allLocations = backendRequire("./SecuityLocations.json")
...
const attachExistingStatus = async (guards) => {
  ...
  const existingEmails = new Set(
    existingGuards.map((guard) => guard.email).filter(Boolean),
  )
  const existingGuardIds = new Set(
    existingGuards.map((guard) => guard.guardId).filter(Boolean),
  )

  return guards.map((guard) => ({
    ...guard,
    exists:
      existingEmails.has(guard.email) || existingGuardIds.has(guard.guardId),
  }))
}
...
const run = async () => {
  ...
  for (const guard of guards) {
    console.log(
      `- ${guard.location} | ${guard.guardId} | ${guard.gender} | ${guard.email} | ${guard.exists ? "skip" : "insert"}`,
    )
  }
