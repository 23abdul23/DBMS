...
export const buildScreenshotFileName = ({ routeName, label, prefix = "aegis" } = {}) => {
...
const requestScreenshotDirectory = async () => {
  ...
  await AsyncStorage.setItem(SCREENSHOT_DIRECTORY_URI_KEY, permission.directoryUri)
  return permission.directoryUri
}

const writeBase64WithStorageAccessFramework = async (base64, fileName) => {
  const storedDirectoryUri = await AsyncStorage.getItem(SCREENSHOT_DIRECTORY_URI_KEY)
  ...
  try {
    if (Platform.OS === "android") {
      try {
        ...
        return await writeBase64WithStorageAccessFramework(capturedBase64, fileName)
      }
    }
