...
const apiBaseUrl = configuredBaseUrl || `http://${fallbackHost}:${fallbackPort}/api`
...
async function run() {
  ...
  try {
    ...
    if (registerResponse.status >= 500) {
      failed = true
      console.error("Registration route returned a server error for an invalid payload.")
      console.error("Expected a 400 validation response if the route and database path are healthy.")
    ...
    failed = true
    console.error("POST /auth/register failed before the server returned a response.")
    console.error(error.message)
  }
