...
export default function CreateOutpassScreen({ navigation }) {
  ...
  const validateForm = () => {
    ...
    if (!longVisit) {
      ...
      if (!isSameDay) {
        Alert.alert(
          'Error',
          'Regular outpasses must start and end on the same day'
        );
        return false;
      }

      if (
        expectedReturn.getHours() > 22 ||
        (expectedReturn.getHours() === 22 && expectedReturn.getMinutes() > 30)
      ) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...formData,
        requestType: formData.longVisit
      ...
      const isLongVisit =
        response?.data?.outpass?.requestType ===
        OUTPASS_REQUEST_TYPE.LONG_VISIT;
      const successMessage = isLongVisit
      ...
      Alert.alert(
        'Error Screen',
        error.response?.data?.message || 'Failed to create outpass'
      );
    } finally {
      setLoading(false);
    }
    ...
    >
      ...
      >
        ...
        <Text
          style={[
            styles.headerTitle,
            { color: colors.heading, flex: 1, textAlign: 'center' },
          ]}
        >
          Create Outpass
        </Text>
        ...
        <View style={styles.form}>
          ...
          <View style={styles.dateTimeContainer}>
            ...
            <View style={styles.dateTimeRow}>
              ...
              >
                ...
                />
                <Text
                  style={[styles.dateTimeText, { color: colors.inputTextInv }]}
                >
          ...
          <TouchableOpacity
            style={[
              localStyles.longVisitToggle,
              {
                ...
                borderColor: formData.longVisit
                  ? colors.warning
                  : colors.inputBorder,
              },
            ]}
          ...
          >
            ...
            <View style={localStyles.longVisitCopy}>
              <Text
                style={[localStyles.longVisitTitle, { color: colors.heading }]}
              >
                Long Visit (Home / Relative Visit)
              </Text>
              <Text
                style={[localStyles.longVisitText, { color: colors.subText }]}
              >
          ...
          {formData.longVisit ? (
            ...
            >
              <Text
                style={[localStyles.noticeTitle, { color: colors.warning }]}
              >
                Physical Warden Approval Required
              </Text>
              <Text style={[localStyles.noticeText, { color: colors.text }]}>
                Submit this request and visit the warden for approval. Once
                approved, it can be used through the normal gate scan flow.
                Same-day and 10:30 PM rules do not apply to this request.
              </Text>
            </View>
          ) : (
            <View
            ...
            >
              <Text
                style={[localStyles.noticeTitle, { color: colors.primary }]}
              >
                Regular Outpass Rule
              </Text>
              <Text style={[localStyles.noticeText, { color: colors.text }]}>
                Regular outpasses must start and end on the same day and return
                by 10:30 PM.
              </Text>
            </View>
          )}
