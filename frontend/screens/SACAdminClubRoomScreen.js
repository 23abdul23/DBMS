      ...
      >
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                >
                  <Ionicons
                    name={
                      entrySource === 'qr' ? 'qr-code-outline' : 'key-outline'
                    }
                    size={26}
                    color={
                ...
                <View style={{ flex: 1, marginLeft: 14 }}>
                  ...
                  >
                    Open a room, join an active room, and see who is currently
                    inside each club room.
                  </Text>
                </View>
              </View>
            </View>
        ...
        >
          ...
          {SAC_CLUB_ROOMS.map((room) => {
            ...
            const userInside = Boolean(
              roomState?.occupants?.some(
                (occupant) => occupant.user?.id === user?.id
              )
            );
            const isBusy =
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  >
                    {roomState?.isOpen
                      ? `${roomState.presentCount} inside${
                          roomState.openedBy
                            ? ` | opened by ${roomState.openedBy.name}`
                            : ''
                        }`
                ...
                <View style={{ padding: 18 }}>
                  {roomState?.isOpen ? (
                        ...
                        {roomState.occupants.map((occupant) => (
                          <View
                            ...
                            style={{
                              ...
                              paddingVertical: 8,
                              backgroundColor:
                                occupant.user?.id === roomState.openedBy?.id
                                  ? colors.warningSoft
                                  : colors.cardMuted,
                              borderWidth: 1,
                              borderColor: colors.border,
                          ...
                          >
                            ...
                            >
                              {occupant.user?.id === roomState.openedBy?.id
                                ? 'Opened room'
                                : `Joined ${formatTime(occupant.joinedAt)}`}
                            </Text>
                          </View>
                        ))}
                      </View>
                    ...
                    >
                      The first student to select this room will be marked as
                      the opener.
                    </Text>
                  )}
