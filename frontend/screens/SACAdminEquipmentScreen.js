        ...
        >
          ...
          {SAC_EQUIPMENT.map((item) => {
            ...
            const userHasItem = Boolean(
              equipmentState?.checkedOutBy?.some(
              ...
              >
                      ...
                      {checkedOutBy.map((entry) => (
                        ...
                        >
                          <View style={{ flex: 1, paddingRight: 12 }}>
                            ...
                            >
                              {entry.user?.studentId || 'Student checkout'}
                              {` • `}
                          ...
                          {canMarkReturned && (
                            <TouchableOpacity
                              disabled={
                                submittingKey === `equipment-return-${entry.id}`
                              }
                              onPress={() =>
                                runAction(
                                  `equipment-return-${entry.id}`,
                                  () =>
                                    sacAPI.returnEquipment(item.name, {
                                      checkoutId: entry.id,
                                    }),
                                  `Unable to return ${item.name} for ${entry.user?.name}.`
                                )
                              }
                              style={{
                                flexDirection: 'row',
                ...
                {!canMarkReturned && checkedOutBy.length > 0 ? (
                  ...
                  >
                    ...
                    >
                      SAC admin access is required to mark these checkouts
                      returned.
                    </Text>
                  </View>
                ) : null}
