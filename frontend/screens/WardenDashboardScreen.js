...
export default function WardenDashboardScreen({ navigation }) {
  ...
  const loadDashboard = async () => {
    try {
      ...
      Alert.alert(
        'Error',
        error?.response?.data?.message || 'Failed to load warden dashboard'
      );
    } finally {
      setLoading(false);
    }
        ...
        <View style={styles.section}>
          ...
          <View style={styles.statGrid}>
            {statCards.map((card) => (
              ...
              >
                <Text style={[styles.statLabel, { color: card.textColor }]}>
                  {card.label}
                </Text>
          ...
          {dashboard?.activeMonitoring?.length ? (
            dashboard.activeMonitoring.map((item) => (
              ...
              >
                ...
                <Text style={[styles.metaText, { color: colors.subText }]}>
                  Return By:{' '}
                  {new Date(item.expectedReturnDate).toLocaleString()}
                </Text>
              </View>
            ))
          ) : (
            <View style={styles.emptyState}>
              ...
              <Text style={[styles.emptyText, { color: colors.subText }]}>
                Students in danger, yellow alert, outside, overdue, or awaiting
                exit will be listed here.
              </Text>
            </View>
          )}
        </View>
