...
import { View, Text, StyleSheet, TouchableOpacity, Alert, ScrollView, Linking, AppState, Platform } from "react-native"
...
export default function EmergencyScreen() {
  ...
  const emergencyTypes = useMemo(
    () => {
      const medicalPhone = getEmergencyPhone("EMERGENCY_MEDICAL_PHONE", "9329594882")
      const securityPhone = getEmergencyPhone("EMERGENCY_SECURITY_PHONE", "7217492629")
      const firePhone = getEmergencyPhone("EMERGENCY_FIRE_PHONE", "86182 75578")
      const otherPhone = getEmergencyPhone("EMERGENCY_OTHER_PHONE", "79090 69340")
  ...
  useEffect(() => {
    const subscription = AppState.addEventListener("change", (nextAppState) => {
      ...
      if (wasInBackground && nextAppState === "active" && pendingSmsRef.current) {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.content}>
        <LocationCard location={location} loading={locationLoading} onRefresh={refreshLocation} />
        ...
        <View style={styles.section}>
          ...
          <Text style={[styles.sectionDescription, { color: colors.text }]}>
            Each button is mapped to a direct number and includes your current location in the SMS draft.
          </Text>
