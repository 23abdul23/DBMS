...
export default function StudentLogsScreen() {
    ...
    try {
      ...
      setError(
        loadError?.response?.data?.message ||
          'Unable to load student logs right now'
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const summaryText = useMemo(() => {
    const outpassCount = logs.filter((item) =>
      isOutpassAction(item.action)
    ).length;
      ...
      <View
        style={[
          localStyles.card,
          {
            ...
            borderColor: outpassRelated
              ? `${actionMeta.color}55`
              : colors.border || COLORS.gray[200],
            borderLeftColor: actionMeta.color,
          },
        ]}
      >
        <View style={localStyles.cardHeader}>
          <Text style={[localStyles.cardTitle, { color: colors.text }]}>
            {actionMeta.label}
          </Text>
          {outpassRelated ? (
            <View
            ...
            >
              <Text style={[localStyles.tagText, { color: actionMeta.color }]}>
                Outpass
              </Text>
            </View>
          ) : null}
        </View>
        <Text style={[localStyles.cardMeta, { color: colors.subText }]}>
          Time: {formatDateTime(item.createdAt)}
        </Text>
        <Text style={[localStyles.cardMeta, { color: colors.text }]}>
        ...
        {item.details?.direction ? (
          <Text style={[localStyles.cardMeta, { color: colors.text }]}>
            Direction: {item.details.direction}
          </Text>
        ) : null}
        {item.details?.nextStatus ? (
          <Text style={[localStyles.cardMeta, { color: colors.text }]}>
            Status: {item.details.nextStatus}
          </Text>
        ) : null}
        {item.details?.reason ? (
          <Text style={[localStyles.cardMeta, { color: colors.text }]}>
      ...
      <View style={[localStyles.container, { backgroundColor: 'transparent' }]}>
        <View
          style={[
            localStyles.header,
            {
              backgroundColor: colors.header + 'F0',
              borderBottomColor: colors.border || COLORS.gray[200],
            },
          ]}
        >
          <Text style={[localStyles.headerTitle, { color: colors.heading }]}>
            My Logs
          </Text>
          {/* <Text style={[localStyles.headerSubtitle, { color: colors.subText }]}>
          Personal entry, exit, and outpass activity
        </Text> */}
          <Text style={[localStyles.headerSummary, { color: colors.subText }]}>
            {summaryText}
          </Text>
