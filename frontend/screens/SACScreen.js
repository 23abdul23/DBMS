...
export default function SACScreen({ navigation, route }) {
  ...
  const loadSacData = useCallback(async (nextLoading = false) => {
    try {
      ...
      if (overviewResult.status === 'fulfilled') {
        ...
        console.log(
          'SAC overview error:',
          overviewResult.reason?.response?.data || overviewResult.reason
        );
        Alert.alert(
          'SAC Error',
          overviewResult.reason?.response?.data?.message ||
            'Unable to load SAC activity right now.'
        );
      }
      ...
      if (statusResult.status === 'fulfilled') {
        ...
        console.log(
          'SAC status error:',
          statusResult.reason?.response?.data || statusResult.reason
        );
        setSacStatus(null);
      }
    } catch (error) {
  ...
  const sacClosesAt =
    sacStatus?.closesAt || overview?.meta?.closesAt || '10:30 PM';
  ...
  const handleNavigateToRooms = () => {
    if (!isSacOpenNow) {
      Alert.alert(
        'SAC Closed',
        sacStatus?.message ||
          `SAC is closed right now. It remains open till ${sacClosesAt}.`,
        [{ text: 'OK' }]
      );
      return;
    }
    navigation.navigate('ClubRooms', {
      entrySource,
      location: route?.params?.location || 'SAC',
    });
  };
          ...
          >
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
                ...
                >
                  <Ionicons
                    ...
                    size={28}
                    color={
                      entrySource === 'qr' ? colors.success : colors.warning
                    }
                  />
                </View>
              ...
              >
                ...
                >
                  Open the club room and equipment sections to manage only your
                  own participation.
                </Text>
              </View>
            </View>
            ...
            >
              ...
              >
                ...
                >
                  Open rooms, see live occupancy counts, and manage your own
                  room membership.
                </Text>
                <Text
                  style={{
              ...
              >
                ...
                >
                  Check live checkout counts and update only the equipment
