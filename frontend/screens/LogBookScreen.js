...
export default function LogBook({ navigation, route }) {
  ...
  const fetchLogs = useCallback(
    async (pageToLoad = 1, replace = true) => {
      ...
      try {
        ...
        setError(
          serverMessage || err?.message || 'Unable to load logs right now'
        );
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  const renderLog = useCallback(
    ({ item }) => {
      ...
      const residentName =
        item?.user?.name || item?.details?.scannedUserName || 'Resident';
      const residentStudentId =
        item?.user?.studentId || item?.details?.scannedStudentId || '-';
      const residentSystemId =
        item?.user?.id || item?.userId || item?.details?.scannedUserId || '-';
      ...
      return (
        <View
          style={[
            styles.logCard,
            { backgroundColor: colors.card, borderLeftColor: actionColor },
          ]}
        >
          ...
          <Text style={[styles.logMeta, { color: colors.text }]}>
            Roll No: {residentStudentId}
          </Text>
          <Text style={[styles.logMeta, { color: colors.text }]}>
            User ID: {residentSystemId}
          </Text>
          <Text style={[styles.logMeta, { color: colors.text }]}>
            Guard: {guardOnDuty}
          </Text>
    ...
    <View style={styles.headerBlock}>
      <View style={styles.summaryBlock}>
        ...
        <Text style={[styles.summaryText, { color: colors.subText }]}>
          Range: {activeRangeText}
        </Text>
        <Text style={[styles.summaryText, { color: colors.subText }]}>
          Showing {logs.length} of {total} record(s)
        </Text>
      ...
      <View style={[styles.filterCard, { backgroundColor: colors.card }]}>
        ...
        <View
          style={[
            styles.searchBox,
            { borderColor: colors.subText, backgroundColor: colors.background },
          ]}
        >
