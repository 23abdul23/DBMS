...
export default function ScanResultCard({ scanResult, location, onClose }) {
  ...
  const variant = useMemo(() => {
    if (isWithoutOutpass) {
      return {
        ...
        subtitle:
          log?.details?.reason ||
          'This student cannot leave campus after 6:00 PM without an approved outpass.',
    ...
    if (action === 'outpass_used' && direction === 'exit') {
      return {
        ...
        title: 'Exit cleared',
        subtitle:
          'Security has verified the approved outpass and logged the student exit.',
        accent: colors.warning,
        soft: colors.warningSoft,
    ...
    if (action === 'exit') {
      return {
        ...
        title: 'Exit recorded',
        subtitle:
          'The scan has been accepted and the movement log is now updated.',
  ...
  const studentId =
    scanResult?.user?.studentId || scanResult?.user?.guardId || '-';
  ...
  const outpassStatus = outpass?.status
  ...
  const warningReason =
    log?.details?.reason || 'This student does not have an approved outpass.';
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCopy}>
          <Text style={[styles.eyebrow, { color: variant.accent }]}>
            {variant.eyebrow}
          </Text>
      ...
      <View style={styles.badgeRow}>
        <View style={[styles.badge, { backgroundColor: variant.soft }]}>
          <Text style={[styles.badgeText, { color: variant.accent }]}>
            {variant.badgeText}
          </Text>
        </View>
      ...
      {showOutpassDetails ? (
        ...
        >
          <View style={styles.outpassHeader}>
            <Text style={[styles.outpassTitle, { color: colors.heading }]}>
              Outpass details
            </Text>
            <View
              style={[styles.statusPill, { backgroundColor: variant.soft }]}
            >
              <Ionicons
                name={isWithoutOutpass ? 'alert-circle' : 'checkmark-circle'}
                size={14}
                color={variant.accent}
              />
              <Text style={[styles.statusPillText, { color: variant.accent }]}>
                {outpassStatus}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.outpassGrid}>
            ...
            <View style={styles.detailBlock}>
              ...
              <Text style={[styles.detailValue, { color: colors.text }]}>
                {formatDateTime(requestedWindow)}
              </Text>
            </View>
            <View style={styles.detailBlock}>
              <Text style={[styles.detailLabel, { color: colors.textMuted }]}>
