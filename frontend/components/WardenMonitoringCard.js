      ...
      <View style={styles.cardRow}>
        <View style={{ flex: 1, paddingRight: 12 }}>
          ...
          <Text style={[styles.metaText, { color: colors.subText }]}>
            {entry.student.studentId || 'No student ID'} | Room{' '}
            {entry.student.roomNumber || '-'}
          </Text>
        </View>
      ...
      {entry.campusPresence === 'outside' ? (
        <>
          <Text style={[styles.metaText, { color: colors.text }]}>
            Exit Gate: {entry.exitGate || '-'}
          </Text>
          <Text style={[styles.metaText, { color: colors.subText }]}>
            Exit Time: {formatDateTime(entry.exitTime)}
          </Text>
          <Text style={[styles.metaText, { color: colors.text }]}>
            Outpass:{' '}
            {entry.hasOutpass ? 'Approved / Used' : 'No approved outpass'}
          </Text>
          {entry.outpass ? (
            <>
              <Text style={[styles.metaText, { color: colors.text }]}>
                Reason: {entry.outpass.reason}
              </Text>
              <Text style={[styles.metaText, { color: colors.text }]}>
                Destination: {entry.outpass.destination}
              </Text>
              <Text style={[styles.metaText, { color: colors.subText }]}>
                Window: {formatDateTime(entry.outpass.outDate)} to{' '}
                {formatDateTime(entry.outpass.expectedReturnDate)}
              </Text>
            </>
          ) : null}
        </>
        ...
        <View style={{ marginTop: 8 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            <Text
              style={[
                styles.metaText,
                { color: colors.text, marginTop: 0, marginLeft: 6 },
              ]}
            >
              Last movement: {formatMovementLine(entry.latestMovement)} on{' '}
              {formatDateTime(entry.latestMovement.createdAt)}
            </Text>
          </View>
            ...
            <View style={{ marginTop: 8 }}>
              {entry.recentMovements.slice(0, 3).map((movement, index) => (
                <Text
                  key={
                    movement.id ||
                    `${movement.action}-${movement.createdAt}-${index}`
                  }
                  style={[
                    styles.metaText,
                    { color: colors.subText, marginTop: index === 0 ? 0 : 4 },
                  ]}
                >
                  {index + 1}. {formatMovementLine(movement)} on{' '}
